// reports.jsx — HR calibration & reporting (bell curve, completed list, CSV/print)
const { useState, useMemo } = React;

const HR_USER = { name: 'Sarah Wijaya', role: 'HR Business Partner', initials: 'SW' };

const CYCLES = [
  { id:'cyc1', name:'Q1 2026 Appraisal',  startDate:'2026-01-01', endDate:'2026-03-31' },
  { id:'cyc2', name:'Q4 2025 Appraisal',  startDate:'2025-10-01', endDate:'2025-12-31' },
  { id:'cyc4', name:'Probation · Mar 2026', startDate:'2026-03-01', endDate:'2026-03-31' },
];
...
function GradeBadge({ g }) {
  ...
  return <span className={`inline-flex h-7 min-w-[36px] items-center justify-center rounded-md text-xs font-bold tabular-nums ${tones[g]||tones.B}`}>{g}</span>;
}
...
function CalibrationModal({ open, appraisal, onClose, onSave }) {
  const [score, setScore] = React.useState('');
  const [grade, setGrade] = React.useState('');
  React.useEffect(() => {
    if (!appraisal) return;
    setScore(appraisal.calibratedScore ?? appraisal.finalScore);
    setGrade(appraisal.finalGrade || gradeFromScore(appraisal.finalScore));
  }, [appraisal, open]);
  if (!open || !appraisal) return null;
  const valid = !isNaN(parseFloat(score)) && score >= 1 && score <= 5 && grade;
  const suggested = gradeFromScore(parseFloat(score) || 0);
  return (
    <Modal open={open} onClose={onClose} size="md"
      title={`Calibrate · ${appraisal.employee}`}
      subtitle={`${appraisal.position} · ${appraisal.dept} · NIP ${appraisal.nip}`}
      footer={
        <>
          ...
          {appraisal.isCalibrated && (
            <button onClick={()=>{ onSave(appraisal.id, null, null); onClose(); }} className="h-9 rounded-lg border border-error-300 bg-white px-4 text-sm font-semibold text-error-600 hover:bg-error-50 dark:border-error-500/40 dark:bg-white/[0.03] dark:text-error-300">Reset calibration</button>
          )}
          <button onClick={()=>{ onSave(appraisal.id, parseFloat(score), grade); onClose(); }} disabled={!valid}
        ...
        <div className="grid grid-cols-3 gap-3 rounded-xl bg-gray-50 p-3 dark:bg-white/[0.02]">
          ...
          <div>
            ...
            <p className="mt-0.5 text-lg font-bold tabular-nums text-gray-900 dark:text-white">
              {appraisal.calibratedScore !== null ? appraisal.calibratedScore.toFixed(2) : <span className="text-gray-400">—</span>}
            </p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider text-gray-500">Status</p>
            <p className="mt-1">
              {appraisal.isCalibrated ? <Badge tone="success">Calibrated</Badge> : <Badge tone="warning">Pending</Badge>}
            </p>
          </div>
        </div>

        <Field label="Calibrated score (1.00 – 5.00)" required hint="Skala sama dengan rating self/reviewer">
          <input type="number" step="0.01" min="1" max="5" value={score}
            onChange={e=>{ setScore(e.target.value); setGrade(gradeFromScore(parseFloat(e.target.value)||0)); }}
            className={inputCls + ' tabular-nums'} />
        </Field>
        <Field label="Final grade" required hint={`Sistem nyaranin: ${suggested} berdasarkan skor`}>
...
function BellCurve({ data }) {
  const max = Math.max(...data.map(d => d.count), 1);
  const palette = { '1.0–1.9':'#f04438', '2.0–2.9':'#f97066', '3.0–3.9':'#fdb022', '4.0–4.4':'#84cc16', '4.5–5.0':'#12b76a' };
...
function App() {
  const [appraisals, setAppraisals] = useState(COMPLETED);
  ...
  const saveCalibration = (id, score, grade) => {
  ...
  const exportCsv = () => {
    ...
    const rows = [header, ...inCycle.map(a => [
      a.nip, a.employee, a.dept, a.position, cycle.name,
      a.finalScore.toFixed(2),
      a.calibratedScore !== null ? a.calibratedScore.toFixed(2) : '',
      a.finalGrade || '',
      a.isCalibrated ? 'calibrated' : 'pending',
    ])];
    const fname = `appraisal-report-${cycle.id}-${new Date().toISOString().slice(0,10)}.csv`;
  ...
  const filters = [
    ...
    { id:'calibrated',  label:'Calibrated', count: stats.calibrated },
  ];
  ...
  return (
    <PageShell active="reports" user={HR_USER} primary={primary} brandHue="indigo">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Cuma menampilkan appraisal ber-status <code>completed</code>. Bell curve pakai calibrated score kalau sudah dikalibrasi.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 no-print">
          <select value={cycleId} onChange={e=>{ setCycleId(e.target.value); setFilter('all'); }}
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.02] print-card">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.02] print-card">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.02] print-card">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-left dark:border-gray-800">
                ...
                <th className="px-3 py-3 text-right">Calibrated</th>
            ...
            <tbody>
              {visible.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-sm text-gray-500 dark:text-gray-400">Tidak ada appraisal untuk filter ini.</td></tr>
              )}
              {visible.map(a => (
                <tr key={a.id} className="border-b border-gray-100 last:border-0 dark:border-gray-800/60">
                  ...
                  <td className="px-3 py-3 text-right tabular-nums text-gray-700 dark:text-gray-300">{a.finalScore.toFixed(2)}</td>
                  ...
                  <td className="px-3 py-3">
                    {a.isCalibrated ? <Badge tone="success">Calibrated</Badge> : <Badge tone="warning">Pending</Badge>}
                  </td>
                  <td className="px-5 py-3 text-right no-print">
                    <button onClick={()=>setEditing(a)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-[11px] font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:bg-white/[0.02] dark:text-gray-200">
      ...
      <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-5 text-xs text-gray-600 dark:border-gray-700 dark:bg-white/[0.02] dark:text-gray-400 no-print">
        ...
        <p className="mt-2">
          Bell curve pakai <code>calibrated_score</code> kalau appraisal sudah dikalibrasi, kalau belum pakai <code>final_score</code>. Print view sudah disiapkan via <kbd>Cmd/Ctrl + P</kbd>.
        </p>
      </div>

      <CalibrationModal open={!!editing} appraisal={editing} onClose={()=>setEditing(null)} onSave={saveCalibration} />
    </PageShell>
  );
}
