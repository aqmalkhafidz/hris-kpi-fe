// sl-review.jsx — Squad Leader (SL) review page
...
const EMPLOYEE = { name:'Aqmal Pratama', initials:'AP', position:'Software Engineer · IC2', squad:'Cart & Checkout' };
const REVIEWER  = { name:'Rifky Oktaviano', initials:'RO', role:'Squad Leader', squad:'Cart & Checkout' };
const CYCLE     = { name:'Q1 2026 Appraisal', short:'Q1 · 2026', end:'Mar 31, 2026' };
const NEXT_REVIEWER = { name:'Dewi Anggraeni', role:'Head of Department' };
...
const INITIAL_KRAS = [
  { id:'k1', title:'Reduce checkout P95 latency',
    desc:'Bring P95 of /checkout below 220ms across our top 3 markets. Own the cart-service refactor and SLO tuning.',
  ...
  { id:'k3', title:'Ship Indonesia payment rails (DANA + GoPay)',
    desc:'GA launch of two e-wallet rails. Vendor contract slipped 3 weeks; mitigation via parallel UAT.',
    ...
    evidence:[], sl_score:0, sl_comment:'' },
  { id:'k4', title:'Reduce on-call alerts by 30%',
    desc:'Tune noisy SLOs and replace cron-driven alerts with anomaly detection on the order pipeline.',
  ...
  { id:'k5', title:'Mentor 2 junior engineers',
    desc:'Weekly 1:1 cadence + design-review shadowing. One mentee submitting promo packet this cycle.',
  ...
  { id:'k6', title:'Reduce flaky-test rate',
    desc:'Quarantine + author-attribution job. Blocked on CI runner upgrade scheduled for May.',
    target:'Flake rate < 1%', weight:10,
    self_score:2, self_comment:'Reached 2.1% — blocked by CI runner upgrade. Quarantine logic shipped; full resolution in Q2.',
    evidence:[], sl_score:0, sl_comment:'' },
];
...
function Avatar({ initials, size='sm', tone='brand' }) {
  ...
  return <div className={`flex shrink-0 items-center justify-center rounded-full font-semibold ${sz} ${tone_cls[tone]||tone_cls.gray}`}>{initials}</div>;
}
...
function SelfCard({ kra }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
      ...
      {kra.self_comment && (
        <p className="mt-2.5 text-sm leading-relaxed text-gray-700">{kra.self_comment}</p>
      )}
...
function KRAReview({ kra, onChange }) {
  return (
    <div>
      ...
      <div className="mt-5"><SelfCard kra={kra} /></div>
      <div className="mt-5 rounded-xl border border-brand-200 bg-brand-50/40 p-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-brand-600">Your review (SL)</p>
        <label className="mb-1.5 block text-sm font-medium text-gray-700">
          Reviewer score <span className="text-error-500">*</span>
        </label>
        <ScorePicker value={kra.sl_score} onChange={v=>onChange({sl_score:v})} />
        <div className="mt-4">
          <label className="mb-1.5 block text-sm font-medium text-gray-700">Comment</label>
          <textarea value={kra.sl_comment} onChange={e=>onChange({sl_comment:e.target.value})} rows={4}
            placeholder="Add observations, context, or calibration notes…"
          ...
          <p className="mt-1 text-xs text-gray-400">{kra.sl_comment.length} / 1000</p>
        </div>
      </div>
    </div>
  );
...
function OverallAssessment({ value, onChange }) {
  return (
    <div>
      ...
      <h2 className="mt-1 font-display text-2xl font-bold tracking-tight text-gray-900">Overall SL review</h2>
      <p className="mt-1 text-sm text-gray-500">Summarize your holistic view of this employee's Q1 2026 performance.</p>
      ...
      <div className="mt-5">
        <label className="mb-1.5 block text-sm font-medium text-gray-700">Overall SL comment</label>
        <textarea value={value} onChange={e=>onChange(e.target.value)} rows={8}
          placeholder="Overall observations, strengths, development areas, and recommendation…"
      ...
      <div className="mt-5 rounded-xl border border-warning-200 bg-warning-50 p-3.5">
        ...
        <p className="mt-0.5 text-sm text-warning-700">
          Once approved, routes to <strong>{NEXT_REVIEWER.name}</strong> ({NEXT_REVIEWER.role}). Score all KRAs first.
        </p>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-gray-900">Send back to employee?</p>
            <p className="text-xs text-gray-500">The employee will be notified to revise and resubmit.</p>
          </div>
        </div>
...
function App() {
  ...
  const reviewedCnt = kras.filter(k=>k.sl_score>0).length;
  const allReviewed = kras.every(k=>k.sl_score>0);
  const canApprove  = allReviewed && overallComment.trim().length > 0;

  const weightedSelf = useMemo(() => {
    const tot = kras.reduce((s,k)=>s+k.weight,0);
    return (kras.reduce((s,k)=>s+(k.self_score*k.weight),0)/tot).toFixed(2);
  }, [kras]);

  const weightedSL = useMemo(() => {
    const scored = kras.filter(k=>k.sl_score>0);
    if (!scored.length) return null;
    const tot = scored.reduce((s,k)=>s+k.weight,0);
    return (scored.reduce((s,k)=>s+(k.sl_score*k.weight),0)/tot).toFixed(2);
  }, [kras]);

  const onPrev = () => {
    if (showOverall) { setOverall(false); setActive(kras[kras.length-1].id); return; }
    const i = kras.findIndex(k=>k.id===activeKraId);
    if (i>0) setActive(kras[i-1].id);
  };
  const onNext = () => {
    if (showOverall) return;
    const i = kras.findIndex(k=>k.id===activeKraId);
    if (i<kras.length-1) setActive(kras[i+1].id);
    else setOverall(true);
  };
  ...
  if (done) return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50">
      <div className="max-w-md rounded-2xl border border-success-200 bg-white p-10 text-center shadow-lg">
        ...
        <p className="mt-2 text-sm text-gray-500">Aqmal's appraisal forwarded to <strong>{NEXT_REVIEWER.name}</strong> for HoD review.</p>
        <a href="KPI Dashboard.html" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-brand-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-600">
          Back to dashboard
        </a>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen">
      <div className="flex min-h-screen">
        ...
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-gray-200 bg-white px-5 py-6 lg:flex">
          ...
          <nav className="mt-2 flex flex-col gap-1">
            <a href="SL Review.html" className="flex items-center gap-3 rounded-xl bg-brand-50 px-3 py-2.5 text-sm font-medium text-brand-700">
              <span className="text-brand-500">{Icon.team}</span>
              <span className="flex-1">Pending Reviews</span>
              <span className="rounded-full bg-brand-100 px-2 py-0.5 text-[10px] font-semibold text-brand-700">1</span>
            </a>
            <button className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50">
              ...
              <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold text-gray-600">12</span>
            </button>
          </nav>
          ...
          <header className="sticky top-0 z-30 border-b border-gray-200 bg-white/85 backdrop-blur">
            <div className="flex h-16 items-center gap-4 px-6">
              ...
              <nav className="hidden items-center gap-1.5 text-xs text-gray-500 md:flex">
                ...
                <span>{Icon.chev}</span><span>SL Review</span>
              </nav>
            ...
            <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-brand-500">SL Review · {CYCLE.short}</p>
                    <h1 className="font-display text-2xl font-bold tracking-tight text-gray-900">{EMPLOYEE.name}</h1>
                    <p className="mt-0.5 text-sm text-gray-500">{EMPLOYEE.position} · {EMPLOYEE.squad}</p>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                  <Badge tone="warning">Pending SL Review</Badge>
                  <div className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs">
                    <span className="text-gray-500">Self avg: </span>
                    <span className="font-semibold tabular-nums text-gray-800">{weightedSelf}</span>
                    {weightedSL && <><span className="mx-1.5 text-gray-300">|</span><span className="text-gray-500">SL avg: </span><span className="font-semibold tabular-nums text-brand-600">{weightedSL}</span></>}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 border-t border-gray-100 pt-4">
                <ApprovalSteps status="sl_review" />
              </div>
            </section>
            ...
            <section className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
              ...
              <aside className="lg:col-span-4 xl:col-span-3">
                <div className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
                    ...
                    return (
                      <button key={k.id} onClick={()=>{setActive(k.id);setOverall(false);}}
                        className={`mb-1.5 flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${
                          isAct?'bg-brand-50/60 ring-1 ring-brand-500/30':'hover:bg-gray-50'
                        }`}>
                        <span className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full ${
                          k.sl_score>0?'bg-success-500 text-white':'border-2 border-dashed border-gray-300 text-gray-400'
                        }`}>{k.sl_score>0?Icon.checkSm:''}</span>
                  ...
                  <button onClick={()=>setOverall(true)}
                    className={`mt-2 flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${
                      showOverall?'bg-brand-50/60 ring-1 ring-brand-500/30':'hover:bg-gray-50'
                    }`}>
                    <span className={`mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full ${
                      overallComment.trim()?'bg-success-500 text-white':'border-2 border-dashed border-gray-300 text-gray-400'
                    }`}>{overallComment.trim()?Icon.checkSm:''}</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-gray-700">Overall Assessment</p>
                      <p className="mt-0.5 text-[11px] text-gray-500">SL comment + approve</p>
                    </div>
                  </button>
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                  ...
                  <ul className="mt-2 space-y-1.5 text-xs">
                    {[
                      [allReviewed, 'All KRAs scored'],
                      [overallComment.trim().length>0, 'Overall comment written'],
                    ].map(([ok, label], i) => (
              ...
              <div className="lg:col-span-8 xl:col-span-9">
                <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm lg:p-8">
                  {!showOverall && active && (
                    <KRAReview kra={active} onChange={p=>update(active.id,p)} />
                  )}
                  {showOverall && (
                    <OverallAssessment value={overallComment} onChange={setOC} />
                  )}
                  <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-5">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={()=>setToast('Draft saved')}
                        className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-4 text-xs font-semibold text-gray-700 hover:bg-gray-50">
                        Save draft
                      </button>
                      <button onClick={()=>setSBO(true)}
                        className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-warning-300 bg-warning-50 px-4 text-xs font-semibold text-warning-700 hover:bg-warning-100">
                      ...
                      <button onClick={()=>{ if(canApprove) setDone(true); else setToast('Score all KRAs and add overall comment first'); }}
                        ...
                        {Icon.send}<span>Approve → HoD</span>
                      </button>
                    </div>
                  </div>
                </div>
      ...
      <SendBackModal
        open={sendBackOpen}
        onConfirm={r=>{setSBO(false);setToast(`Sent back to ${EMPLOYEE.name}`);}}
