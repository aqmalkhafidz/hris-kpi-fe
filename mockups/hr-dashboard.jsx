// hr-dashboard.jsx — HR-side overview
...
const RECENT_SUBMISSIONS = [
  { who:'Aqmal Hidayat',     team:'Engineering · Backend',  to:'Squad Leader review', when:'2 min ago',  initials:'AH', tone:'brand' },
  { who:'Kirana Andini',     team:'Design · Product',       to:'HoD review',           when:'14 min ago', initials:'KA', tone:'success' },
  { who:'Bagas Widodo',      team:'Operations · Logistics', to:'Squad Leader review', when:'38 min ago', initials:'BW', tone:'warning' },
  ...
  { who:'Mira Lestari',      team:'Finance · Reporting',    to:'Squad Leader review', when:'3 h ago',    initials:'ML', tone:'gray' },
];

const ATTENTION = [
  { kind:'overdue',  title:'12 employees missed self-appraisal deadline', subtitle:'Mar 24 deadline · auto-reminder sent at 09:00 today', tone:'error' },
  { kind:'stalled',  title:'7 reviews stuck in Squad Leader stage > 5 days', subtitle:'Engineering (4), Operations (2), Marketing (1)',     tone:'warning' },
  { kind:'calibrate',title:'Calibration meeting scheduled Apr 8',           subtitle:'Heads of Division · 14:00–16:30 · Meeting Room Garuda', tone:'brand' },
];

const SCORE_BUCKETS = [
  { label:'1.0–1.9',  count:2,   tone:'#f04438' },
  { label:'2.0–2.9',  count:9,   tone:'#f97066' },
  { label:'3.0–3.9',  count:48,  tone:'#fdb022' },
  { label:'4.0–4.4',  count:71,  tone:'#84cc16' },
  { label:'4.5–5.0',  count:32,  tone:'#12b76a' },
];

// === Components ===
function StatCard({ icon, label, value, sub, delta, tone='brand' }) {
  const tones = {
      ...
      <div className="flex items-center justify-between">
        ...
        {delta && (
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${
            ...
            {delta.dir==='up' ? Icon.arrowUp : Icon.arrowDn}{delta.value}
          </span>
        )}
      </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.02]">
      <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Hover a row for breakdown · click name to drill down</p>
        </div>
        <div className="flex items-center gap-2">
          <select value={sortBy} onChange={e=>setSortBy(e.target.value)}
            className="h-9 rounded-lg border border-gray-200 bg-white px-3 text-xs text-gray-600 focus:border-brand-300 focus:outline-none focus:ring-4 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-300">
                ...
                <tr key={r.name} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/60 dark:border-gray-800/60 dark:hover:bg-white/[0.02]">
                  ...
                  <td className="px-3 py-4 text-right">
                    <span className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-semibold tabular-nums ${
                      r.avg >= 4 ? 'bg-success-50 text-success-700 dark:bg-success-500/10 dark:text-success-400'
                                  : 'bg-warning-50 text-warning-700 dark:bg-warning-500/10 dark:text-warning-400'
                    }`}>{r.avg.toFixed(2)}</span>
                  </td>
                  <td className="px-3 py-4">
                    <div className="flex justify-end gap-1.5 text-[11px] tabular-nums text-gray-500 dark:text-gray-400">
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.02]">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Final scores · {total} appraisals reviewed so far</p>
        </div>
        <Badge tone="gray">5-pt scale</Badge>
      </div>
        ...
        {SCORE_BUCKETS.map(b => {
          ...
          return (
            <div key={b.label} className="flex flex-1 flex-col items-center gap-2">
              ...
              <div className="w-full rounded-t-lg" style={{ height: h, background: b.tone, boxShadow: `0 -2px 0 0 ${b.tone}33 inset` }} />
      ...
      <ul className="mt-4 space-y-3">
        {ATTENTION.map((a,i) => (
          <li key={i} className="flex items-start gap-3 rounded-xl border border-gray-200 px-3 py-3 dark:border-gray-800">
            <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${tones[a.tone]}`}>{Icon.warn}</span>
            ...
            <button className="rounded-lg border border-gray-200 px-2.5 py-1 text-[11px] font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-white/[0.05]">Action</button>
          </li>
        ))}
      </ul>
    </div>
          ...
          <li key={i} className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                {s.who} <span className="font-normal text-gray-500 dark:text-gray-400">moved to</span> {s.to}
              </p>
...
function CycleHeader() {
  const completionPct = Math.round(((CYCLE.completed + CYCLE.hodivApproved + CYCLE.hodApproved) / CYCLE.invited) * 100);
  return (
      ...
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Self-appraisal closes <strong className="text-gray-700 dark:text-gray-200">{CYCLE.selfDeadline}</strong> · final calibration <strong className="text-gray-700 dark:text-gray-200">Apr 8</strong>
          </p>
        </div>
            ...
            <div className="mt-2 flex items-center gap-3">
              <div className="relative h-14 w-14">
                <svg viewBox="0 0 36 36" className="h-14 w-14 -rotate-90">
                  <circle cx="18" cy="18" r="15.9" fill="none" stroke="currentColor" className="text-gray-200 dark:text-gray-800" strokeWidth="3"/>
                  <circle cx="18" cy="18" r="15.9" fill="none" className="text-brand-500" stroke="currentColor" strokeWidth="3"
                    strokeDasharray={`${completionPct}, 100`} strokeLinecap="round"/>
                </svg>
                <span className="absolute inset-0 flex items-center justify-center text-sm font-bold tabular-nums text-gray-900 dark:text-white">{completionPct}%</span>
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{CYCLE.completed + CYCLE.hodivApproved + CYCLE.hodApproved}/{CYCLE.invited}</p>
...
function App() {
  const primary = (
    <button className="hidden md:inline-flex h-10 items-center gap-2 rounded-xl bg-brand-500 px-4 text-sm font-semibold text-white shadow-sm hover:bg-brand-600">
      {Icon.plus}<span>New cycle</span>
    </button>
  );
  return (
    <PageShell active="hr_dashboard" user={HR_USER} primary={primary} brandHue="indigo">
      <CycleHeader />
