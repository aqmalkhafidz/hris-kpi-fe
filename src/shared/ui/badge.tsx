...
interface BadgeProps {
  children: ReactNode
  tone?: 'gray' | 'success' | 'warning' | 'error' | 'brand' | 'info' | 'neutral'
  size?: 'sm' | 'md'
}
...
export function Badge({ children, tone = 'gray', size = 'sm' }: BadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full font-medium ${sizes[size]} ${tones[tone] ?? tones.gray}`}>
      {children}
    </span>
  )
}
