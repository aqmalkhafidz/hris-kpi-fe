...
export function ScoreComparison({ kra, roles }: { kra: Kra; roles: Role[] }) {
  return (
    <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${roles.length}, minmax(0, 1fr))` }}>
      {roles.map(role => {
        ...
        return (
          ...
          >
            ...
            <p className={`mt-0.5 text-lg font-bold ${filled ? 'text-brand-700 dark:text-brand-300' : 'text-gray-300 dark:text-gray-600'}`}>
              {filled ? value : '—'}
            </p>
          </div>
        )
