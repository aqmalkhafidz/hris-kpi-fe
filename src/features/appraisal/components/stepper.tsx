        ...
        return (
          <div key={step.id} className="flex flex-1 items-center last:flex-none">
            <div className="flex items-center gap-2">
              ...
              <span className={`hidden text-xs font-medium sm:inline ${active ? 'text-brand-700 dark:text-brand-300' : 'text-gray-500 dark:text-gray-400'}`}>{step.label}</span>
            </div>
            {index < steps.length - 1 && <div className={`mx-3 h-px flex-1 ${index < current ? 'bg-success-500' : 'bg-gray-200 dark:bg-gray-800'}`} />}
          </div>
        )
      })}
    </div>
