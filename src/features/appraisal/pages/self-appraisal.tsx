...
import { lastReturnEntry, type Kra } from '@shared/lib/types/appraisal';
...
import { SCORE_LABELS, type KraDraft } from '../constants';
import {
  useAdvanceAppraisal,
  useMyAppraisals,
  useSubmitAppraisal,
...
export function SelfAppraisalPage() {
  ...
  const allScored =
    draftKras.length > 0 && filledKras.length === draftKras.length;
  const currentReflection = reflection || appraisal?.reflection || '';
  const reflectionFilled = currentReflection.trim().length > 0;
  const completion = draftKras.length
    ? Math.round(
        ((filledKras.length + (reflectionFilled ? 1 : 0)) /
          (draftKras.length + 1)) *
          100
      )
    : 0;
  ...
  const saveDraft = async () => {
    ...
    await submitMut.mutateAsync({
      ...
      updates: { kras: collectKras(), reflection: currentReflection },
    });
    setSaving(false);
  ...
  if (!appraisal) {
    return (
      <PageShell breadcrumb="Self-Appraisal">
        <EmptyState title="No active appraisal found." />
      </PageShell>
    );
  }
    ...
    <PageShell breadcrumb="Self-Appraisal">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                ...
                <span className="font-semibold tabular-nums text-gray-700 dark:text-gray-300">
                  {filledKras.length}/{draftKras.length} KRAs · reflection{' '}
                  {reflectionFilled ? 'complete' : 'pending'} · {completion}%
                </span>
              </div>
      ...
      {!editable && (
        <section className="rounded-2xl border border-warning-100 bg-warning-50 px-5 py-4 text-sm text-warning-700 dark:border-warning-500/20 dark:bg-warning-500/10 dark:text-warning-300">
          This appraisal is already in{' '}
          <strong>{appraisal.status.replace(/_/g, ' ')}</strong>. Editing is
          locked.
        </section>
      )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 text-sm">
              <p className="font-semibold text-warning-800 dark:text-warning-200">
                Returned by {returnNote.actor_name} (
                {returnNote.actor_role.toUpperCase()})
              </p>
              <p className="mt-1 text-warning-700 dark:text-warning-300">
                {returnNote.reason}
              </p>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        <aside className="space-y-4 lg:col-span-4 xl:col-span-3">
          <SectionCard>
            <p className="mb-3 text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">
              KRA list · {filledKras.length}/{draftKras.length} scored
            </p>
            <div className="space-y-1.5">
              {draftKras.map((kra, index) => {
                const isActive = !showReflection && active?.id === kra.id;
                const done =
                  kra.self_score > 0 && kra.self_comment.trim().length > 0;
                return (
                  <button
                    key={kra.id}
                    type="button"
                    onClick={() => {
                      setActiveKraId(kra.id);
                      setShowReflection(false);
                    }}
              ...
              >
                <span
                  className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${reflectionFilled ? 'bg-success-500 text-white' : 'border border-dashed border-gray-300 text-gray-400 dark:border-gray-700'}`}
                >
          ...
          <SectionCard title="Submit checklist">
            <ul className="space-y-2">
              ...
              <ChecklistItem done={reflectionFilled}>
                Reflection written
              </ChecklistItem>
              <ChecklistItem done={totalWeight === 100}>
                KRA weight totals 100%
              </ChecklistItem>
            </ul>
            {user?.role === 'sl' && (
        ...
        <section className="lg:col-span-8 xl:col-span-9">
          <SectionCard
            ...
            description={
              showReflection
                ? 'Step back from the KRAs and summarize the cycle.'
                : active
                  ? `Target: ${active.target} · Weight ${active.weight}%`
                  : undefined
            }
            action={
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {SCORE_LABELS[active.self_score] ?? 'Choose a score'}
                    </p>
                  </div>
                  <ScorePicker
                    value={active.self_score}
                    disabled={!editable}
                    onChange={(score) => patchKra(active, { score })}
                  />
                </div>
                ...
                <FormField label="Achievement narrative">
                  <Textarea
                    ...
                    onChange={(event) =>
                      patchKra(active, { comment: event.target.value })
                    }
                ...
                <div className="space-y-3">
                  ...
                  <EvidenceAdder
                    disabled={!editable}
                    onAdd={(item) =>
                      patchKra(active, { evidence: [...active.evidence, item] })
                    }
                  />
                </div>
              </div>
