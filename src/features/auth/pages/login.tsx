...
function BrandMark({ mobile = false }: { mobile?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <div
        className={`${mobile ? 'h-9 w-9 rounded-xl' : 'h-10 w-10 rounded-xl'} flex items-center justify-center bg-brand-500 text-white shadow-lg shadow-brand-500/35`}
      >
      ...
      <div>
        <p
          className={`${mobile ? 'text-base text-gray-900 dark:text-white' : 'text-base text-white'} font-bold tracking-tight`}
        >
          Performa
        </p>
        {!mobile && (
          <p className="mt-0.5 text-[11px] font-medium uppercase text-white/40">
            HR Console
          </p>
        )}
      </div>
    ...
    <div className="flex min-h-screen bg-[var(--bg-page)]">
      <aside className="login-brand relative hidden min-h-screen w-[44%] shrink-0 flex-col overflow-hidden bg-[#11152a] p-12 text-white md:flex">
        ...
        <div className="relative my-auto max-w-sm">
          <h1 className="text-[46px] font-semibold leading-none text-white">
            Performance,
            <br />
            Simplified.
          </h1>
          <p className="mt-6 text-sm leading-7 text-white/55">
            One platform for appraisal cycles, KRA templates, and team
          ...
          <div className="mt-10 space-y-4">
            {[
              ...
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-full border border-brand-400/50 bg-brand-500/25 text-brand-100">
                  {Icon.check}
                </span>
                {feature}
              </div>
        ...
        <p className="relative text-xs text-white/25">
          © 2026 Performa · All rights reserved
        </p>
      </aside>
        ...
        <div className="w-full max-w-[400px]">
          ...
          <div className="mb-9">
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Use the Quick Login dropdown to test any role.
            </p>
          </div>
          ...
          >
            ...
            <label className="flex cursor-pointer select-none items-center gap-2.5 text-sm text-gray-700 dark:text-gray-300">
              <input
                ...
                className="h-5 w-5 rounded-md border-gray-300 text-brand-500 focus:ring-brand-500"
              />
